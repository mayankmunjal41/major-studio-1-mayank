...
d3.select('.wheel').on('click', function() {
  ...
  d3.select('.close').style('top', '30px')
                ...
                d3.selectAll('.swatch > div').on('click', function() {
                  ...
                  if (allColorsFilled) {
                    ...
                    d3.select('.save').on('click', function () {
                      ...
                      selectedSwatches.push({ fills: masterPalette });
...
setTimeout(() => {
  ...
  .attr("y", (height / 2) - margin.bottom / 2 - 100)
  .style("fill", "white")
  .style("font-size", "14px")
  .text("Click on different colors")
  .append("tspan")
  .attr("x", xScale(dataSet[44].beginyear) + margin.left + 10)
  .attr("y", (height / 2) - margin.bottom / 2 - 80)
...
d3.select('.click')
  ...
  svg.append("image")
    ...
    .attr("x", xScale(dataSet[42].beginyear) + margin.left + 45)
    .attr("y", (height / 2) - margin.bottom / 2 - 75)
    .attr("width", 40)
    .attr("height", 40)
    .attr("xlink:href", "line1.svg")
    .style('opacity', 0)
    .transition().duration(1000)
    .style('opacity', 1);
...
// Create buttons
const buttons = [
  { id: "renaissanceButton", label: "Renaissance (1400-1600)", filterRange: [1400, 1600] },
  { id: "baroqueButton", label: "Baroque (1600-1750)", filterRange: [1600, 1750] },
  { id: "romanticButton", label: "Romantic (1800-1850)", filterRange: [1800, 1850] },
  { id: "modernismButton", label: "Early Modernism (1850-1900)", filterRange: [1850, 1900] },
...
function filterData(range) {
  ...
  // Update the x-axis
  let xAxis = d3.axisBottom(xScale).tickValues([1400, 1600, 1750, 1800, 1850]);
  ...
  // Update the existing circles with new data
  let countriesCircles = svg.selectAll(".countries").data(filteredData, function (d) {
    return d.country;
  });
